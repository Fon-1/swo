 ...
 * @author maian
 */
@WebServlet(name = "CreateBeneficiaryPersonServlet", urlPatterns = {"/createbeneficiaryperson"})
public class CreateBeneficiaryPersonServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CreateBeneficiaryPersonServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CreateBeneficiaryPersonServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (staff == null) {
            ...
            try {
                ...
                BeneficiaryPerson benePerson = new BeneficiaryPerson(fullName,
                        gender, dob, cccd, nationality, maritalStatus, education,
                        job, address, phoneNumber, email, relationship);

                session.setAttribute("benePerson", benePerson);

                if (!checkDOB(dob)) {
                    request.setAttribute("dobError", "Người thụ hưởng phải trên 30 ngày tuổi.");
                ...
                request.getRequestDispatcher("createcontract").forward(request, response);
