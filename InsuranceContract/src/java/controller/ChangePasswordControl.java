        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Customer user = (Customer) session.getAttribute("authcode");
             if(!user.getPassword().equals(oldpassword)){
                ...
                String errorMessage = "Mật khẩu của bạn không hợp lệ! Vui lòng thử lại.";
                request.setAttribute("errorMessage", errorMessage);
                request.getRequestDispatcher("ChangePassword.jsp").forward(request, response);
            }
             else if (!password.equals(repassword)) {
                session.setAttribute("authcode", user);
                String errorMessage = "Mật khẩu mới và xác thực mật khẩu mới phải trùng nhau! Vui lòng thử lại.";
                request.setAttribute("errorMessage", errorMessage);
                request.getRequestDispatcher("ChangePassword.jsp").forward(request, response);
            } else if(user.getPassword().equals(oldpassword) && password.equals(repassword))  {
                SendEmail sm = new SendEmail();
                String code = sm.getRandom();
                Customer a = new Customer(user.getCustomerID(), code, user.getEmail(), password);
